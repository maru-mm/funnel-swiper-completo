...
/** Prompt preferiti: cliccando si incolla il testo nell'area istruzioni */
const FAVORITE_PROMPTS: { label: string; text: string }[] = [
  {
    label: "Mantieni codice/stile",
    text: "Keep this code and style unchanged. Return the exact same HTML: same structure, classes, inline styles and layout. Do not alter design or content. Only fix invalid HTML syntax if any. Preserve all structure, classes, styling and markup. Only fix invalid HTML or indentation if needed.",
  },
];
        ...
        !isSelectedVariantError && (
          ...
          >
            <div className="grid w-full gap-2 relative">
              ...
              />
              {FAVORITE_PROMPTS.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  <span className="text-xs text-gray-500 self-center mr-1">Prompt preferiti:</span>
                  {FAVORITE_PROMPTS.map((fav) => (
                    <Button
                      key={fav.label}
                      type="button"
                      variant="outline"
                      ...
                      onClick={() => setUpdateInstruction(fav.text)}
                    >
                      {fav.label}
                    </Button>
                  ))}
