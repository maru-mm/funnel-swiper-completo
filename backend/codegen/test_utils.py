...
class TestUtils(unittest.TestCase):
    ...
    def test_extract_html_content_with_partial_html_tags(self):
        """Truncated HTML (no </html>) gets </body></html> appended."""
    ...
    def test_markdown_tags(self):
        """No <html> in content: fences are stripped then raw text is returned."""
        text = "```html<head></head>```"
        result = extract_html_content(text)
    ...
    def test_truncated_html_append_closing_tags(self):
        """When output is truncated (has <html> but no </html>), append </body></html>."""
        text = '<html><head><meta charset="utf-8"></head><body><div>content</div>'
        result = extract_html_content(text)
